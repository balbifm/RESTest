...
//		originalTest.equals(testCase);
        ...
        getBodyGenerator(testOperation).ifPresent(g -> generationAlternatives.add("request_body_mutation"));
    ...
    Optional<ITestDataGenerator> getBodyGenerator(Operation testOperation) {
        if (testOperation.getOpenApiOperation().getRequestBody() != null) {
            ...
            if (bodyParam != null) {
                ...
                return s.filter(x -> x instanceof BodyGenerator || x instanceof ObjectPerturbator)
                        .findFirst();
            }
        }
    ...
    protected TestCase createTestCaseTemplate(Operation testOperation) {
        String testId = "test_" + removeNotAlfanumericCharacters(testOperation.getOperationId())
                + "_" + testOperation.getMethod() + "_" + IDGenerator.generateId();
